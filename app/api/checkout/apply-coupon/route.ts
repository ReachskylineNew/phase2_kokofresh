...
/**
 * Apply coupon/promo code to checkout
 */
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { checkoutId, couponCode } = body;

    if (!checkoutId || !couponCode) {
      return NextResponse.json(
        { error: "checkoutId and couponCode are required" },
        { status: 400 }
      );
    }
    ...
    try {
      // Apply coupon using Wix checkout API
      ...
      return NextResponse.json(
        {
          error: error.message || "Failed to apply coupon. Please check the code and try again.",
